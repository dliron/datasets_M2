...
sp500     = xts(sp500[,2]   , order.by = as.Date(sp500[,1]   ))
brent     = xts(brent [,2]  , order.by = as.Date(brent [,1]  ))
usbond10  = xts(usbond10[,2], order.by = as.Date(usbond10[,1]))
eurusd    = xts(eurusd[,2]  , order.by = as.Date(eurusd[,1]  ))
uscpi     = xts(uscpi[,2]   , order.by = as.Date(uscpi[,1]   ))
usgdp     = xts(usgdp[,2]   , order.by = as.Date(usgdp[,1]   ))
...
base$usinfl = na.locf(base$usinfl)
base$ustgdp = na.locf(base$ustgdp)
...
bb_up = BBands(sp500)[,'up']
bb_dn = BBands(sp500)[,'dn']

xx = cbind(sp500,bb_up,bb_dn)["2020/"]
plot(xx)
